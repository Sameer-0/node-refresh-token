...
module.exports = class Faculties {
    ...
    static fetchAll(rowcount) {
        return poolConnection.then(pool => {
            return pool.request().query(`SELECT TOP ${Number(10)} fs.id, fs.faculty_id, fs.faculty_name,camp.campus_abbr, org.org_abbr, sit.start_time, sit.end_time, fs.is_processed FROM [dbo].faculties fs
            INNER JOIN [dbo].campuses camp ON camp.campus_id = fs.campus_lid
            INNER JOIN [dbo].slot_interval_timings sit ON sit.id = fs.start_time_id
            INNER JOIN [dbo].organizations org ON org.org_id =  org_lid WHERE sit.active = 1  AND camp.active = 1 AND org.active = 1`)
        })
    }
