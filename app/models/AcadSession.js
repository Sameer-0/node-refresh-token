    ...
    static search(rowcount, keyword) {
        return poolConnection.then(pool => {
            ...
            return request.input('keyword', sql.NVarChar(100), '%' + keyword + '%')
                .query(`SELECT TOP ${Number(rowcount)} ac.id AS acadSessionId, ac.acad_session,  ac.sap_acad_session_id FROM [dbo].acad_sessions ac WHERE ac.acad_session LIKE @keyword ORDER BY ac.id DESC`)
        })
    }
