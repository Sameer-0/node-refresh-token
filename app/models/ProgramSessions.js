...
module.exports = class {
    static fetchAll(rowcount, slug) {
        return poolConnection.then(pool => {
            return pool.request().query(`SELECT TOP ${Number(rowcount)} ps.id, p.program_name, p.program_code, adc.acad_session, ps.active FROM [${slug}].program_sessions ps 
    ...
    static pagination(rowcount, pageNo, slug) {
        return poolConnection.then(pool => {
            ...
            return request.input('pageNo', sql.Int, pageNo)
                .query(`SELECT ps.id, p.program_name, p.program_code, adc.acad_session, ps.active FROM [${slug}].program_sessions ps 
                INNER JOIN [${slug}].programs p ON ps.program_lid = p.id
                INNER JOIN [dbo].acad_sessions adc ON adc.id = ps.acad_session_lid
    ...
    static search(rowcount, keyword, slug) {
        return poolConnection.then(pool => {
            return pool.request().input('keyword', sql.NVarChar(100), '%' + keyword + '%')
                .query(`SELECT TOP ${Number(rowcount)} ps.id, p.program_name, p.program_code, adc.acad_session, ps.active FROM [${slug}].program_sessions ps 
                INNER JOIN [${slug}].programs p ON ps.program_lid = p.id
                INNER JOIN [dbo].acad_sessions adc ON adc.id = ps.acad_session_lid
